...
public class VentanaConsultaPartidosController implements Initializable {
    ...
    public void consultarPartidos() {
        ...
        try {
            ...
            if (partido.getequipoVisitante().equals((String) equipo2CB.getValue())) {
                ...
                try ( FileInputStream fs = new FileInputStream(App.pathImagesBanderas + partido.getNombreImgEquipoLocal())) {
                ...
                try ( FileInputStream fs = new FileInputStream(App.pathImagesBanderas + partido.getNombreImgEquipoVisitante())) {
    ...
    public void verDetallesEquipos() {
        ...
        mensaje.setStyle("-fx-font-size:25;");
        ...
        for (Jugador jug : jugadoresEquipoLocal) {
            try ( FileInputStream fs = new FileInputStream(App.pathImages + "carga.png")) {

                VBox infoJugadorVbox = new VBox();

                ImageView imagen = new ImageView();
                imagen.setImage(new Image(fs));
                imagen.setFitHeight(100);
                imagen.setFitWidth(100);
                Label nombreJugador = new Label("cargando");
                ...
                //Simulacion de asicronia de carga de imagenes
                imagenHilo(jug, infoJugadorVbox);
        ...
        tituloEqLocal.setStyle("-fx-font-size:15;");
        ...
        equipoVisitanteFp.setOrientation(Orientation.HORIZONTAL);

        for (Jugador jug : jugadoresEquipoVisitante) {
            try ( FileInputStream fs = new FileInputStream(App.pathImages + "carga.png")) {

                VBox infoJugadorVbox = new VBox();
                ImageView imagen = new ImageView(new Image(fs));
                imagen.setFitHeight(100);
                imagen.setFitWidth(100);
                Label nombreJugador = new Label("cargando");
                infoJugadorVbox.setPrefSize(130, 230);
        ...
        tituloEqVisitante.setStyle("-fx-font-size:15;");
    ...
    public void imagenHilo(Jugador jug, VBox infoJugadorVbox) {
        //Abrir un hilo por cada imagen
        Thread hiloImagen = new Thread(new Runnable() {
            @Override
            public void run() {
                try ( FileInputStream fs = new FileInputStream(App.pathImagesJugadores + jug.getNombreImg())) {
                    ImageView imagen = new ImageView();
                    imagen.setImage(new Image(fs));
                    imagen.setFitHeight(100);
                    imagen.setFitWidth(100);
                    ...
                    Random r = new Random();
                    int numAleatorio = r.nextInt((10 - 1) + 1) + 1;
                    //Dormir el hilo
                    Thread.sleep(numAleatorio * 1000);
                    //Actualizar la ui
                    Platform.runLater(() -> {
                        infoJugadorVbox.getChildren().clear();
                        infoJugadorVbox.getChildren().addAll(imagen, nombreJugador);
                    });

                } catch (Exception e) {
                    System.out.println("no es posible abir las imagenes de los jugadores");
                }
            }
        });

        hiloImagen.setDaemon(true);
        hiloImagen.start();
    }
